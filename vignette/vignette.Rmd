...
```{bash}
variant_qc.R tmp/example.gds --out_prefix tmp/ --ms_xmax 0.12
mv tmp/*.png fig
```
...
```{r init_qc_keep}
dplyr::filter(varmet, missing.rate < 0.05, !(maf < 0.05 & maf > 0))$variant.id %>%
...
outputs a vector of LD-pruned variant IDs `tmp/pruned_snps.rds`.

**bash:**

```{bash}
ld_pruning.R tmp/example.gds --out_prefix tmp/ \
...
```{bash}
king_grm.R tmp/example.gds --out_prefix tmp/ --variant_id tmp/pruned_snps.rds \
...
```{bash}
kinship_plot.R tmp/king_out.rds --is_king --out_file fig/kinship.png
```
...
```{bash}
pcair.R tmp/example.gds tmp/king_grm.rds tmp/king_grm.rds \
  --out_prefix tmp/a_ --variant_id tmp/pruned_snps.rds --num_core 12
```
...
phen <- file.path(data_dir, pheno_dir, "2009_08_24_phenotype_data.csv") %>%
  read.csv(as.is = TRUE) %>%
  dplyr::mutate_at(dplyr::vars(SUBJ_NO, Race), as.character)
# pca_plots.R expects a variable named sample.id, so rename before writing to disk
phen_adf <- dplyr::rename(phen, sample.id = SUBJ_NO) %>%
...
```{bash}
pca_plots.R tmp/a_pcair.rds --out_prefix fig/a_ --group Race --n_pairs 4 \
...
PCs, so the resulting GRM only represents recent relatedness. This script saves
the following in the `tmp/` directory:
...
```{bash}
pcrelate.R tmp/example.gds tmp/a_pcair.rds --out_prefix tmp/a_ --n_pcs 3 \
  --variant_id tmp/pruned_snps.rds
```
...
```{bash}
pcair.R tmp/example.gds tmp/a_pcr_mat.rds tmp/king_grm.rds \
  --out_prefix tmp/b_ --variant_id tmp/pruned_snps.rds --num_core 12
pcrelate.R tmp/example.gds tmp/b_pcair.rds --out_prefix tmp/b_ \
  --n_pcs 3 --variant_id tmp/pruned_snps.rds --scale_kin 2
```
...
phen.sub <- "/nfs/beluga0_home/ANALYSIS/NIAGADS/NG00020/pheno/out/pheno_adf.rds" %>%
  readRDS() %>%
  Biobase::pData() %>%
  dplyr::select(sample.id, Case_Control, SEX, Race, Hispanic, age, E2, E4) %>%
  dplyr::mutate(across(Case_Control, as.integer))
```
...
sexcheck <- blPipeline::read_sexcheck("tmp/plink.sexcheck") %>%
  dplyr::full_join(phen.sub, by = c(IID = "sample.id")) %>%
  dplyr::select(1:6, SEX)
```
...
```{r compare_fam_pheno}
dplyr::select(sexcheck, PEDSEX, SEX) %>% table(useNA = "ifany")
```
...
```{r sexcheck_status}
dplyr::filter(sexcheck, STATUS == "PROBLEM")
```
...
```{r sex_mismatch}
sex_mismatch <- dplyr::filter(sexcheck, PEDSEX != SNPSEX, SNPSEX != 0)$IID
...
pheno <- phen.sub %>%
  dplyr::left_join(pcs, "sample.id") %>%
  dplyr::filter(sample.id %in% gds.id)
...
dd <- file.path(data_dir, pheno_dir, "2009_07_24_data_dictionary.csv") %>%
  read.csv(as.is = TRUE)
dd.sub <- dplyr::filter(dd, VARNAME %in% names(phen.sub)) %>%
    dplyr::select(VARNAME, VARDESC, VALUES)
dd.sub
...
-v tmp/pruned_snps.rds: A vector of variant IDs to keep, in .rds format
...
let's create a keep list that excludes samples with mismatched sex.

```{r pheno_keep}
sex_exclude <- readRDS("tmp/sex_mismatch.rds")
samp.keep <- dplyr::filter(pheno, !(sample.id %in% sex_exclude))$sample.id
...
```{bash}
fit_null_mod.R tmp/pheno_adf.rds --grm_file tmp/b_pcr_mat.rds \
  --sample_id tmp/samp.keep.rds --outcome martingale --family gaussian \
  --out_file tmp/nullmod.rds --covars sex E2 E4 pc1 pc2 pc3
```
...
```{bash}
association_test.R tmp/example.gds tmp/pheno_adf.rds --out_prefix tmp/ \
  --sample_id tmp/samp.keep.rds --variant_id tmp/vars_qc_keep.rds \
