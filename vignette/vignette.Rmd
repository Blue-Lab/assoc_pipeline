...
"Degree 3", etc (e.g. avuncular, first cousin, etc), and we re-label them to
...
length(msm_rm_ids)
saveRDS(samp_pass_qc$sample.id[!(samp_pass_qc$sample.id %in% msm_rm_ids)],
        "vignette_out/kinship_errors_rm.rds")
```
...
```{bash}
plink \
  --bfile /nfs/beluga0_home/ANALYSIS/VIGNETTE/vignette_data/out/vignette \
  --out tmp/plink \
  --noweb \
  --check-sex
```
