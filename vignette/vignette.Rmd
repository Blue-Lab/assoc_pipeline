...
GDS.  This vignette will use a subset of the `NG00020_plink` data that's been
subsetted to a random 100,000 variants. 
...
If we also have a pedigree, we can compare the GRM to expected relationships.
We'll only consider relationships of degree 3 or higher; as you can see from
the above plot, the boundaries between less-related groups grow blurrier and
blurrier in the bottom-right corner.

Define the cutpoints for different relationships.
...
Read in the pedigree and format it in a way that's compatible with
`GWASTools::pedigreeCheck()`.
...
We can convert the KING GRM into a pairwise-relationships format, and create
relationship categories based on the thresholds above.

```{r king-rels}
king <- readRDS("vignette_out/king_out.rds") %>%
...
Before proceeding, we need to resolve any internal inconsistencies in the
pedigree. This pedigree has a common formatting issue where single-person
...
Filtering out these families, we can convert the pedigree to a similar pairwise
relationship format. Note that relationships calculated with
`GWASTools::pedigreePairwiseRelatedness()` are much more precise than "Degree 2",
"Degree 3", etc (e.g. avuncular, second cousin, etc), and we re-label them to
match the above KING-inferred relationships.
...
Join the two datasets together to compare. Below is a frequency table of the
KING and pedigree relationships.

```{r kin-compare}
kin_compare <- filter(rels, Individ1 %in% c(king$ID1, king$ID2),
                      Individ2 %in% c(king$ID1, king$ID2)) %>%
  full_join(king, c(Individ1 = "ID1", Individ2 = "ID2"),
            suffix = c(".ped", ".king")) %>%
...
Plot of relatives of degree 2 or higher (a pairwise list of relationships
converted from a GRM can have thousands or millions of rows, and can be very
slow to plot; restricting to just close relatives can speed this step up).
...
Plot of just relationships where the KING-inferred value differs from the
expected value from the pedigree. The red point in the top left represents a
likely sample swap; it is very clearly a parent-offspring relationship, but
was expected to be degree 2 relationship.

The remaining relationship mismatches are less clearly indicative of sample
swaps. They are all near the cutoff between degrees 2, 3 and "unrelated", and
we would expect to see such a pattern in a plot like this.
...
length(msm_rm_ids)
saveRDS(all_missing_rm[!(all_missing_rm %in% msm_rm_ids)], "vignette_out/kinship_errors_rm.rds")
```
...
The pairwise plots turn into clouds after about the seventh PC and PCs 8 and
greater account for almost no variance, so we'll use 7 for the next step.
...
The pairwise PC plots look similar. Continue with 7 PCs.
...
`tmp/vars_ruth_rmd.rds` that we just prepared, but
