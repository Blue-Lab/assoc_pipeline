...
```{r read_sexchec}
sexcheck <- read_sexcheck("tmp/plink.sexcheck") %>%
...
gds <- seqOpen("tmp/example.gds")
gds.id <- seqGetData(gds, "sample.id")
pheno <- phen.sub %>%
  left_join(pcs, "sample.id") %>%
...
names of the age, case status, and covariate variables to `blPipeline::make_resids()`.

```{r make_resids}
covars <- c("SEX", "E2", "E4", "pc1", "pc2", "pc3")
pheno %<>% make_resids("age", "Case_Control", covars = covars)
```
...
```{r}
pheno_adf <- AnnotatedDataFrame(pheno)
meta <- setNames(dd.sub$VARDESC, dd.sub$VARNAME)
varMetadata(pheno_adf)[names(meta), "labelDescription"] <- meta

## rename to match expected values for SeqVarTools
varLabels(pheno_adf)[varLabels(pheno_adf) == "SEX"] <- "sex"

## check by constructing a SeqVarData object
seqData <- SeqVarData(gds, pheno_adf)
...
samp.keep <- filter(pheno, !(sample.id %in% sex_exclude))$sample.id
