...
argp <- arg_parser("Variant QC") %>%
    ...
    add_argument("--ms_xmin", help = "Variant-missingness plot x-axis lower limit") %>%
    add_argument("--ms_xmax", help = "Variant-missingness plot x-axis upper limit") %>%
    add_argument("--ms_ymin", help = "Variant-missingness plot y-axis lower limit") %>%
    add_argument("--ms_ymax", help = "Variant-missingness plot y-axis upper limit") %>%
    add_argument("--maf_xmin", help = "MAF plot x-axis lower limit") %>%
    add_argument("--maf_xmax", help = "MAF plot x-axis upper limit") %>%
    add_argument("--maf_ymin", help = "MAF plot y-axis lower limit") %>%
    add_argument("--maf_ymax", help = "MAF plot y-axis upper limit")
...
p_ms <- ggplot(var.df, aes(missing.rate)) +
    ...
    lims(x = c(argv$ms_xmin, argv$ms_xmax), y = c(argv$ms_ymin, argv$ms_ymax))
ggsave(paste0(argv$out_prefix, "missing_by_variant.", argv$img_ext), plot = p_ms)

p_maf <- ggplot(var.df, aes(maf)) +
    geom_histogram(binwidth=0.01, boundary=0) +
    lims(x = c(argv$maf_xmin, argv$maf_xmax), y = c(argv$maf_ymin, argv$maf_ymax))
